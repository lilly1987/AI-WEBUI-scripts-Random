...
class Script(scripts.Script):
    ...
    def run(self, p, step1, step2, stepc, cfg1, cfg2, cfgc, draw_legend, include_lone_images, no_fixed_seeds,xychg):
        print(f"{step1};{step2};{stepc};{cfg1};{cfg2};{cfgc};{xychg}")
        print(f"{type(step1)};{type(step2)};{type(stepc)};{type(cfg1)};{type(cfg2)};{type(cfgc)};{type(xychg)}")
        ...
        def process_axis(opt, vals):
            ...
            return valslist
        '''
        def process_axis(v1, v2, v3,opt):
                
            if v1 > v2 :
                valslist = [random.randint(v2,v1) for r in range(v3)]
            else :
                valslist = [random.randint(v1,v2) for r in range(v3)]
        ...
        if xychg :
            x_opt = axis_options[5]
            #xs = process_axis(x_opt, x_values)
            xs = process_axis(step1, step2, stepc,x_opt)
            
            y_opt = axis_options[4]
            #ys = process_axis(y_opt, y_values)
            ys = process_axis(cfg1, cfg2, cfgc,y_opt)
        else:
            y_opt = axis_options[5]
            #xs = process_axis(x_opt, x_values)
            ys = process_axis(cfg1, cfg2, cfgc,y_opt)
            
            x_opt = axis_options[4]
            #ys = process_axis(y_opt, y_values)
            xs = process_axis(step1, step2, stepc,x_opt)
        ...
        if opts.grid_save:
            images.save_image(processed.images[0], p.outpath_grids, "rnd_grid", prompt=p.prompt, seed=processed.seed, grid=True, p=p)
